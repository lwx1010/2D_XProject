...
// #endif
			...
			else
			{
				...
				catch (System.ArgumentException ex)
				{
                    ...
                    msg.Append("Error calling ");

					if (mTarget == null) msg.Append(mMethod.Name);
					else msg.Append(mTarget.GetType()).Append(".").Append(mMethod.Name);
					...
					{
						msg.Append(mParameterInfos[0]);
						for (int i = 1; i < mParameterInfos.Length; ++i)
							msg.Append(", ").Append(mParameterInfos[i].ParameterType);
					}
					...
					{
						msg.Append(mParameters[0].type);
						for (int i = 1; i < mParameters.Length; ++i)
							msg.Append(", ").Append(mParameters[i].type);
					}
					msg.Append("\n");
