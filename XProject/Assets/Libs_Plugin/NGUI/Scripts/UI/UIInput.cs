...
public class UIInput : MonoBehaviour
{
	...
	public string Validate (string val)
	{
        ...
        int length = Encoding.Unicode.GetByteCount(sb.ToString());
	    int characterLimitUnicode = characterLimit*2;
        if (characterLimitUnicode > 0 && length > characterLimitUnicode)
	    {
	        string maxChars = sb.ToString();
            while (length > characterLimitUnicode)
            {
                maxChars = maxChars.Substring(0, maxChars.Length - 1);
                length = Encoding.Unicode.GetByteCount(maxChars);
            }
            return maxChars;
        }
	...
	protected virtual void Insert (string text)
	{
		...
		int rl = Encoding.UTF8.GetByteCount(right);
		...
		for (int i = 0, imax = text.Length; i < imax; ++i)
		{
            ...
            int length = Encoding.Unicode.GetByteCount(sb.ToString());
            if (characterLimit > 0 && length + rl >= characterLimit * 2) break;
	...
	protected char Validate (string text, int pos, char ch)
	{
        ...
        else if (validation == Validation.Positive_integer)
        {
