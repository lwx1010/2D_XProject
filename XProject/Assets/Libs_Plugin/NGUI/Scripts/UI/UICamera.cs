...
public class UICamera : MonoBehaviour
{
    ...
    static public T Raycast<T>(Vector3 inPos) where T : Component
    {
        for (int i = 0; i < list.size; ++i)
        {
            UICamera cam = list.buffer[i];
            ...
            if (cam.eventType == EventType.World_3D)
            {
                if (Physics.Raycast(ray, out lastHit, dist, mask))
                {
                    GameObject hoveredObject = lastHit.collider.gameObject;
                    ...
                    T comp = hoveredObject.GetComponent<T>();
                    if(comp)    return comp;
                }
            }
            else if (cam.eventType == EventType.UI_3D)
            ...
            else if (cam.eventType == EventType.World_2D)
            {
                if (m2DPlane.Raycast(ray, out dist))
                {
                    ...
                    if (c2d)
                    {
                        GameObject hoveredObject = c2d.gameObject;
            ...
            else if (cam.eventType == EventType.UI_2D)
            {
                ...
                if (m2DPlane.Raycast(ray, out dist))
                {
                    Collider2D[] hits = Physics2D.OverlapPointAll(point, mask);
	...
	{
	    this.tooltipDelay = 5;
	    this.mouseClickThreshold = 80;
	    this.touchClickThreshold = 80;
...
//        AntiAliasing aa = transform.GetOrAddComponent<AntiAliasing>();
//        aa.method = (int)AntiAliasing.Method.Smaa;

//	    AntiAliasing aa = transform.GetComponent<AntiAliasing>();
//	    if (aa != null)
//	    {
//            if(Application.isPlaying)
//                GameObject.Destroy(aa);
//            else
//                GameObject.DestroyImmediate(aa);
//	    }
    }
	...
	public void ProcessMouse ()
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			else if (Input.GetMouseButton(i) || Input.GetMouseButtonUp(i))
			{
				currentScheme = ControlScheme.Mouse;
				isPressed = true;
			}
		...
		if (currentScheme == ControlScheme.Touch || !isPressed) return;
	...
	public void ProcessTouches ()
	{
		...
		for (int i = 0; i < count; ++i)
		{
			...
			// Raycast into the screen
			if (pressed && !Raycast(currentTouch.pos)) hoveredObject = fallThrough;
			if (hoveredObject == null) hoveredObject = mGenericHandler;
    ...
    private void processPosition(bool isMouseDown, bool isMouseUp, Vector3 screenPos)
    {
        // Is any button currently pressed?
        bool isPressed = false;
        bool justPressed = false;
        ...
        {
            mNextRaycast = RealTime.time + 0.02f;
            if (!Raycast(screenPos)) hoveredObject = fallThrough;
            if (hoveredObject == null) hoveredObject = mGenericHandler;
            for (int i = 0; i < 3; ++i) mMouse[i].current = hoveredObject;
        }
        ...
        for (int i = 0; i < 3; ++i)
        {
            bool pressed = i == 0 && isMouseDown;
            bool unpressed = i == 0 && isMouseUp;
            if (pressed || unpressed) currentScheme = ControlScheme.Mouse;
            currentTouch = mMouse[i];
    ...
    public IEnumerator ProcessScreenPostion(Vector3 screenPos)
    {
        this.processPosition(true, false, screenPos);

        yield return Yielders.GetWaitForSeconds(0.2f);

        this.processPosition(false, true, screenPos + Vector3.one * 0.1f);
    ...
    public static void HideCullMask(string layerName)
    {
        CameraUtil.HideLayerName(list[0].cachedCamera , layerName);
    }

    public static void ShowCullMask(string layerName)
    {
        CameraUtil.ShowLayerName(list[0].cachedCamera, layerName);
    }
}
