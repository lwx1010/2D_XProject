...
public class CutsceneCreatorWindow : EditorWindow
{
    ...
    const string MENU_ITEM = "Tools/剧情编辑器/Create Cutscene";
    const string MENU_TRIGGER_ITEM = "Tools/剧情编辑器/触发器Trigger";
    #endregion
    ...
    [MenuItem(MENU_TRIGGER_ITEM , false , 11)]
...
//            }
            ...
            if (GUILayout.Button("Create Cutscene"))
            {
                ...
                GameObject plotCamGO = new GameObject("PlotCamera");
                plotCamGO.AddComponent<PlotCamera>();
                DestroyImmediate(plotCamGO.GetComponent<FlareLayer>());
                DestroyImmediate(plotCamGO.GetComponent<GUILayer>());
                DestroyImmediate(plotCamGO.GetComponent<AutoUpdateScene>());
                plotCamGO.transform.SetParent(cutsceneGO.transform);
                ...
                if(StartMethod != StartMethod.None)
                {
                    ...
                    cutsceneTrigger.CutName = cutsceneName;
