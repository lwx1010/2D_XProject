            ...
            try {
                ...
                if (frames != null && frames.Length > 0) {
                    ...
                    for (int i = 0; i < count; i++) {
                        ...
                        if (frame.CustomStartsWith ("System.Collections.Generic.") || frame.CustomStartsWith ("ShimEnumerator")) {
                            continue;
                        }
                        if (frame.CustomStartsWith ("Bugly")) {
                            continue;
                        }
    ...
    private static void _HandleException (LogSeverity logLevel, string name, string message, string stackTrace, bool uncaught)
    {
        ...
        if (!string.IsNullOrEmpty (message)) {
            try {
                ...
                } else if ((LogSeverity.LogError == logLevel) && message.CustomStartsWith ("Unhandled Exception:")) {
