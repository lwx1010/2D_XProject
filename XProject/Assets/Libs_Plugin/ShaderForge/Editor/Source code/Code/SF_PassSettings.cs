...
namespace ShaderForge
{
    ...
    public class SF_PassSettings : ScriptableObject
    {
        ...
        public SF_PassSettings Initialize(SF_Editor materialEditor)
        {
            ...
            cats.Add(catMeta = NewCat<SFPSC_Meta>("着色器设置"));
            cats.Add(catProperties = NewCat<SFPSC_Properties>("属性"));
            cats.Add(catLighting = NewCat<SFPSC_Lighting>("光照"));
            cats.Add(catGeometry = NewCat<SFPSC_Geometry>("几何"));
            cats.Add(catBlending = NewCat<SFPSC_Blending>("混合"));
            cats.Add(catExperimental = NewCat<SFPSC_Experimental>("实验功能"));
            cats.Add(catConsole = NewCat<SFPSC_Console>("控制台"));
