    ...
    public class SF_NodeTreeStatus : ScriptableObject
    {
        ...
        public bool CheckCanCompile()
        {
            ...
            foreach (SF_Node n in cNodes)
            {
                ...
                if (n is SFN_Tex2d || n is SFN_Cubemap)
                { // Check MIP input
			...
			*/
            ...
            if (osx && !ogl)
            {
                SF_ErrorEntry error = SF_ErrorEntry.Create("您的着色器无法在您的工作区上正常呈现 - 在 OSX 中工作时，您需要启用 OpenGL。 点击图标启用 OpenGL！", true);
                error.action = () =>
                {
            ...
            else if (windows)
            {
                if (inDx11Mode && !dx11)
                {
                    SF_ErrorEntry error = SF_ErrorEntry.Create("您的着色器无法在您的工作区上正常呈现 - 在 Windows 上以 DX11 模式工作时，您需要启用 Direct3D 11。 点击图标启用 Direct3D 11！", true);
                    error.action = () =>
                    {
                ...
                {
                    SF_ErrorEntry error = SF_ErrorEntry.Create("您的着色器无法在您的工作区上正常呈现 - 在 Windows 上工作时，需要启用 Direct3D 9。 点击图标启用 Direct3D 9！", true);
                    error.action = () =>
                    {
            ...
            {
                SF_ErrorEntry error = SF_ErrorEntry.Create("您正在使用 Face Sign 节点，但是您的着色器不是双面的。 点击图标修复", false);
                error.action = () =>
                {
