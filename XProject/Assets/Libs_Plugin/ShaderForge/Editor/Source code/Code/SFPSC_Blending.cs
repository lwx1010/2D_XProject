...
namespace ShaderForge
{
    ...
    public class SFPSC_Blending : SFPS_Category
    {
        ...
        public static string[] strDepthTestStencil = new string[] { "<", ">", "\u2264", "\u2265", "=", "\u2260", "一直（默认）", "永不" };
        public static string[] strStencilOp = new string[] { "保持（默认）", "归零", "替换", "反转", "增多（强制）", "减少（强制）", "增多（覆盖）", "减少（覆盖）" };
        public static string[] strRenderType = new string[] { "无", "不透明", "透明", "透明抠图", "背景", "覆盖", "树不透明", "树透明抠图", "树公告牌", "草", "草公告牌" };
        public static string[] strDepthTest = new string[] { "<", ">", "\u2264（默认）", "\u2265", "=", "\u2260", "一直" };
        public static int[] queueNumbers = new int[] { 1000, 2000, 2450, 3000, 4000 };
        public static string[] strQueue = new string[] { "背景 (1000)", "不透明几何 (2000)", "Alpha 裁剪 (2450)", "透明 (3000)", "叠加 (4000)" };
        public static string[] strDithering = new string[] { "关闭", "2x2 矩阵", "3x3 矩阵", "4x4 矩阵" };
        ...
        public void StencilBlock(ref Rect r)
        {
            ...
            useStencilBuffer = GUI.Toggle(r, useStencilBuffer, useStencilBuffer ? "模板缓冲区" : "模板缓冲区...");
            ...
            allowStencilWriteThroughProperties = UndoableToggle(r, allowStencilWriteThroughProperties, "公开模板作为属性", "toggle expose stencil as properties");
            r.y += 20;
            ...
            Rect rTmp = r;
            rTmp.width = 88;
            GUI.Label(rTmp, "参考值", EditorStyles.miniLabel);
            ...
            stencilComparison = (DepthTestStencil)UndoableLabeledEnumPopupNamed(rTmp.PadRight(4).ClampWidth(32, 140), "对比", stencilComparison, strDepthTestStencil, "stencil comparison");
            r.y += 20;

            StencilBitfield(r, "读取遮罩", ref stencilMaskRead);
            r.y += 20;
            StencilBitfield(r, "写入遮罩", ref stencilMaskWrite);
            r.y += 23;
            stencilPass = (StencilOp)UndoableLabeledEnumPopupNamed(r.PadRight(4), "通过", stencilPass, strStencilOp, "stencil pass");
            r.y += 20;
            stencilFail = (StencilOp)UndoableLabeledEnumPopupNamed(r.PadRight(4), "失败", stencilFail, strStencilOp, "stencil fail");
            r.y += 20;
            stencilFailZ = (StencilOp)UndoableLabeledEnumPopupNamed(r.PadRight(4), "失败 Z", stencilFailZ, strStencilOp, "stencil fail Z");
            r.y += 20;
            r.xMin -= 20;
            r.y += 20;
        ...
        public void FogBlock(ref Rect r)
        {
            ...
            useFog = UndoableToggle(r, useFog, "接受雾", "receive fog", null);
            r.y += 20;
            ...
            GUI.enabled = useFog;
            {
                ...
                r.height = 17;
                fogColor = EditorGUI.ColorField(r, "叠加雾的颜色", fogColor);//SF_GUI.LabeledColorField(r,"Override Fog Color",fogDensity,EditorStyles.miniLabel);
                r.height = 20;
