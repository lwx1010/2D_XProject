    ...
    public class SF_FeatureChecker : ScriptableObject
    {
        ...
        public void UpdateAvailability()
        {
            ...
            if (ps.catLighting.lightMode == SFPSC_Lighting.LightMode.PBL)
            {
                ...
                {
                    editor.mainNode.diffuse.label = "Base Color 基本颜色";
                    editor.mainNode.specular.label = "Metallic 金属";
                }
                else
                {
                    editor.mainNode.diffuse.label = "Albedo 反射率";
                    editor.mainNode.specular.label = "Specular 镜面反射（高光）";
                }
            }
            else
            {
                editor.mainNode.diffuse.label = "Diffuse 漫反射";
                editor.mainNode.specular.label = "Specular 镜面反射（高光）";
            }
            ...
            {
                editor.mainNode.gloss.label = "Roughness 粗糙";
            }
            else
            {
                editor.mainNode.gloss.label = "Gloss 光泽";
            }

            if (ps.catGeometry.vertexOffsetMode == SFPSC_Geometry.VertexOffsetMode.Relative)
            {
                editor.mainNode.vertexOffset.label = "Vertex Offset 顶点偏移";
            }
            else if (ps.catGeometry.vertexOffsetMode == SFPSC_Geometry.VertexOffsetMode.Absolute)
            {
                editor.mainNode.vertexOffset.label = "Vertex Position 顶点位置";
            }
