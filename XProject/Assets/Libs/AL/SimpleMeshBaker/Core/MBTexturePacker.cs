...
//						img.y = r.y;
		...
		bool Probe(Image[] imgsToAdd, int idealAtlasW, int idealAtlasH, float imgArea, int maxAtlasDim, ProbeResult pr){
			...
			for (int i = 0; i < imgsToAdd.Length; i++){
				...
				if (n == null){
					...
					if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Probe success efficiency w=" + usedW + " h=" + usedH + " e=" + efficiency + " sq=" + squareness + " fits=" + fitsInMaxDim);
					return true;
				}
			}	
		...
		Rect[] _GetRects(List<Vector2> imgWidthHeights, int maxDimension, int padding, int minImageSizeX, int minImageSizeY, int masterImageSizeX, int masterImageSizeY, out int outW, out int outH, int recursionDepth){
			if (LOG_LEVEL >= MBLogLevel.debug) Debug.Log (String.Format("_GetRects numImages={0}, maxDimension={1}, padding={2}, minImageSizeX={3}, minImageSizeY={4}, masterImageSizeX={5}, masterImageSizeY={6}, recursionDepth={7}",
			...
			if ((float)maxH/(float)maxW > 2){
				if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Using height Comparer");
				Array.Sort(imgsToAdd,new ImageHeightComparer());
			}
			else if ((float)maxH/(float)maxW < .5){
				if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Using width Comparer");
				Array.Sort(imgsToAdd,new ImageWidthComparer());
			}
			else{
				if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Using area Comparer");
			...
			while (numWIterations >= 1 && steppedHeight < sqrtArea * 1000){	
				...
				while (!successW && steppedWidth < sqrtArea * 1000){	
					...
					if (LOG_LEVEL >= MBLogLevel.trace) Debug.Log ("Probing h=" + steppedHeight + " w=" + steppedWidth);
					if (Probe(imgsToAdd, steppedWidth, steppedHeight, area, maxDimension, pr)){
						...
						if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("increasing Width h=" + steppedHeight + " w=" + steppedWidth);
					}			
				}
				steppedHeight = StepWidthHeight(steppedHeight,stepH,maxDimension);
				if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("increasing Height h=" + steppedHeight + " w=" + steppedWidth);
			}
			...
			if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Best fit found: atlasW=" + outW + " atlasH" + outH + " w=" + bestRoot.w + " h=" + bestRoot.h + " efficiency=" + bestRoot.efficiency + " squareness=" + bestRoot.squareness + " fits in max dimension=" + bestRoot.fitsInMaxSize);
			...
			if (bestRoot.w > maxDimension){
				...
				if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Packing exceeded atlas width shrinking to " + scaleFactor);
				for (int i = 0; i < images.Count; i++){
					...
					if (im.w * scaleFactor < masterImageSizeX){ //check if small images will be rounded too small. If so need to redo packing forcing a larger min size
						if (LOG_LEVEL >= MBLogLevel.debug) Debug.Log ("Small images are being scaled to zero. Will need to redo packing with larger minTexSizeX.");
						redoPacking = true;
			...
			if (bestRoot.h > maxDimension){
				...
				if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Packing exceeded atlas height shrinking to " + scaleFactor);
				for (int i = 0; i < images.Count; i++){
					Image im = images[i];
					if (im.h * scaleFactor < masterImageSizeY){ //check if small images will be rounded too small. If so need to redo packing forcing a larger min size
						if (LOG_LEVEL >= MBLogLevel.debug) Debug.Log ("Small images are being scaled to zero. Will need to redo packing with larger minTexSizeY.");
						redoPacking = true;
			...
			if (!redoPacking){
				...
				for (int i = 0; i < images.Count; i++){
					...
					if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Image: " + i + " imgID=" + im.imgId + " x=" + r.x * outW +
							   " y=" + r.y * outH + " w=" + r.width * outW +
						       " h=" + r.height * outH + " padding=" + padding);
				}
			} else {
				if (LOG_LEVEL >= MBLogLevel.debug) Debug.Log ("==================== REDOING PACKING ================");
				bestRoot = null;
			...
			if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Done GetRects");
			return rs;			
		}
	}
}