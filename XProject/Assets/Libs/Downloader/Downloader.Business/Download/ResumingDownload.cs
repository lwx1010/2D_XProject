    ...
    public class ResumingDownload : AbstractDownload
    {
        ...
        private void CheckHeartbeat()
        {
            while (true)
            {
                ...
                lock (this.monitor)
                {
                    ...
                    {
                        if (this.currentRetry > 0) this.currentRetry = 0;
                    }
                }
            }
        }
        ...
        private void CountRetryAndCancelIfMaxRetriesReached()
        {
            if (this.maxRetries.HasValue && this.currentRetry >= this.maxRetries)
            {
                ...
                this.OnDownloadCancelled(new DownloadCancelledEventArgs(this, new TooManyRetriesException("Too many retries, please check your network status")));
