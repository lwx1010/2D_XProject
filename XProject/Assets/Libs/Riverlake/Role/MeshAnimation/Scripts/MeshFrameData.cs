...
namespace FSG.MeshAnimator
{
	...
	public class DeltaCompressedFrameData
	{
		...
		public static implicit operator DeltaCompressedFrameData(MeshFrameData[] frames)
		{
			...
			for (int i = 0; i < frames.Length; i++)
			{
			    if (frames[i].rootMotionPosition.x != 0 ||frames[i].rootMotionPosition.y != 0 ||frames[i].rootMotionPosition.z != 0 ||
			        frames[i].rootMotionRotation.x != 0 ||frames[i].rootMotionRotation.y != 0 ||
			        frames[i].rootMotionRotation.z != 0 ||frames[i].rootMotionRotation.w != 0)
			    {
                    hasRootMotion = true;
			        break;
			    }
