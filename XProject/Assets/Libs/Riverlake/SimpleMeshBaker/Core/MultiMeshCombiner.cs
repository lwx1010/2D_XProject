...
namespace Riverlake.SMB.Core{
    ...
    /// </summary>
    public class MultiMeshCombiner: MeshCombiner {
	
	    public override MBLogLevel LOG_LEVEL{
	    ...
	    public override MBValidationLevel validationLevel{
		    set{
	    ...
	    public class CombinedMesh{
		    public MeshCombinerSingle combinedMesh;
	    ...
	    public override bool AddDeleteGameObjectsByID(GameObject[] gos, int[] deleteGOinstanceIDs, bool disableRendererInSource=true){
		    ...
		    if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("MB2_MultiMeshCombiner.AddDeleteGameObjects numCombinedMeshes: " + meshCombiners.Count + " added:" + gos + " deleted:" + deleteGOinstanceIDs + " disableRendererInSource:" + disableRendererInSource + " maxVertsPerCombined:" + _maxVertsInMesh);		
		    return _bakeStep1(gos, deleteGOinstanceIDs, disableRendererInSource);
	    }
	
	    bool _validate(GameObject[] gos, int[] deleteGOinstanceIDs){
		    if (_validationLevel == MBValidationLevel.none) return true;
		    ...
		    if (gos != null){
			    for (int i = 0; i < gos.Length; i++){
				    ...
				    if (_validationLevel >= MBValidationLevel.robust){
	    ...
	    void _distributeAmongBakers(GameObject[] gos, int[] deleteGOinstanceIDs){
		    ...
		    for (int i = 0; i < deleteGOinstanceIDs.Length; i++){
			    ...
			    if (obj2MeshCombinerMap.TryGetValue(deleteGOinstanceIDs[i], out c)){
				    if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("MB2_MultiMeshCombiner.Removing " + deleteGOinstanceIDs[i] + " from meshCombiner " + meshCombiners.IndexOf(c));
		    ...
		    for (int i = 0; i < gos.Length; i++){
			    ...
			    for (int j = 0; j < meshCombiners.Count; j++){
				    if (meshCombiners[j].extraSpace + meshCombiners[j].numVertsInListToDelete - meshCombiners[j].numVertsInListToAdd > numVerts){
					    cm = meshCombiners[j];
					    if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("MB2_MultiMeshCombiner.Added " + gos[i] + " to combinedMesh " + j, LOG_LEVEL);					
					    break;					
				    }
			    }
			    if (cm == null){
				    ...
				    if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("MB2_MultiMeshCombiner.Created new combinedMesh");
			    }
			    cm.gosToAdd.Add(go);
	    ...
	    bool _bakeStep1(GameObject[] gos, int[] deleteGOinstanceIDs, bool disableRendererInSource){
		    ...
		    for (int i = 0; i < meshCombiners.Count; i++){
			    ...
			    if (cm.combinedMesh.targetRenderer == null){
				    ...
				    if (_LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("BuildSO combiner {0} goID {1} targetRenID {2} meshID {3}",i,cm.combinedMesh.targetRenderer.gameObject.GetInstanceID(), cm.combinedMesh.targetRenderer.GetInstanceID(), cm.combinedMesh.GetMesh().GetInstanceID());
			    ...
			    if (cm.gosToAdd.Count > 0 || cm.gosToDelete.Count > 0){
				    ...
				    if (_LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Baked combiner {0} obsAdded {1} objsRemoved {2} goID {3} targetRenID {4} meshID {5}",i,cm.gosToAdd.Count,cm.gosToDelete.Count,cm.combinedMesh.targetRenderer.gameObject.GetInstanceID(), cm.combinedMesh.targetRenderer.GetInstanceID(), cm.combinedMesh.GetMesh().GetInstanceID());
			    }
            ...
            for (int combinerIdx = 0; combinerIdx < meshCombiners.Count; combinerIdx++)
                {
                    ...
                    {
                        MeshCombinerSingle.MBBlendShape bs = meshCombiners[combinerIdx].combinedMesh.blendShapes[i];
	    ...
	    public override void DestroyMesh(){
		    for (int i = 0; i < meshCombiners.Count; i++){
			    if (meshCombiners[i].combinedMesh.targetRenderer != null){
				    MeshBakerUtility.Destroy(meshCombiners[i].combinedMesh.targetRenderer.gameObject);
			    }
