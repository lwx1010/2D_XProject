...
namespace Riverlake.SMB.Core
{
    ...
    /// </summary>
    public class MeshCombinerSingle : MeshCombiner
    {
        /*
		 Stores information about one source game object that has been added to
		 the combined mesh.  
		*/
        public class DynamicGameObject : IComparable<DynamicGameObject>
        {
			...
			*/
            ...
            public int CompareTo(DynamicGameObject b)
            {
        ...
        public class MeshChannelsCache
        {
            ...
            Color[] _getMeshColors(Mesh m)
            {
                ...
                if (cs.Length == 0)
                {
                    if (this.mc.LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Mesh " + m + " has no colors. Generating");
                    if (this.mc.LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Mesh " + m + " didn't have colors. Generating an array of white colors");
            ...
            Vector3[] _getMeshNormals(Mesh m)
            {
                ...
                if (ns.Length == 0)
                {
                    if (this.mc.LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Mesh " + m + " has no normals. Generating");
                    if (this.mc.LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Mesh " + m + " didn't have normals. Generating normals.");
            ...
            Vector4[] _getMeshTangents(Mesh m)
            {
                ...
                if (ts.Length == 0)
                {
                    if (this.mc.LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Mesh " + m + " has no tangents. Generating");
                    if (this.mc.LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Mesh " + m + " didn't have tangents. Generating tangents.");
            ...
            Vector2[] _getMeshUVs(Mesh m)
            {
                ...
                if (uv.Length == 0)
                {
                    if (this.mc.LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Mesh " + m + " has no uvs. Generating");
                    if (this.mc.LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Mesh " + m + " didn't have uvs. Generating uvs.");
            ...
            Vector2[] _getMeshUV2s(Mesh m)
            {
                ...
                if (uv.Length == 0)
                {
                    if (this.mc.LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Mesh " + m + " has no uv2s. Generating");
                    if (this.mc.LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Mesh " + m + " didn't have uv2s. Generating uv2s.");
                    uv = new Vector2[m.vertexCount];
                    for (int i = 0; i < uv.Length; i++) { uv[i] = _HALF_UV; }
                }
                return uv;
        ...
        public override TextureBakeResults textureBakeResults
        {
            set
            {
                ...
                {
                    if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("If material bake result is changed then objects currently in combined mesh may be invalid.");
                }
        ...
        public override GameObject resultSceneObject
        {
            set
            {
                if (_resultSceneObject != value)
                {
                    ...
                    if (_mesh != null && _LOG_LEVEL >= MBLogLevel.warn)
                    {
        ...
        List<DynamicGameObject> mbDynamicObjectsInCombinedMesh = new List<DynamicGameObject>();
        Dictionary<int, DynamicGameObject> _instance2combined_map = new Dictionary<int, DynamicGameObject>();
        ...
        DynamicGameObject instance2Combined_MapGet(int gameObjectID)
        {
            return _instance2combined_map[gameObjectID];
        }

        void instance2Combined_MapAdd(int gameObjectID, DynamicGameObject dgo)
        {
        ...
        bool instance2Combined_MapTryGetValue(int gameObjectID, out DynamicGameObject dgo)
        {
        ...
        void _initialize()
        {
            ...
            if (_mesh == null)
            {
                if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("_initialize Creating new Mesh");
                _mesh = GetMesh();
            }
                ...
                boneWeights.Length > 0)
            {
                ...
                for (int i = 0; i < mbDynamicObjectsInCombinedMesh.Count; i++)
                {
                    DynamicGameObject dgo = mbDynamicObjectsInCombinedMesh[i];
        ...
        bool _collectMaterialTriangles(Mesh m, DynamicGameObject dgo, Material[] sharedMaterials, OrderedDictionary sourceMats2submeshIdx_map)
        {
            ...
            for (int i = 0; i < numTriMeshes; i++)
            {
                ...
                if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Collecting triangles for: " + dgo.name + " submesh:" + i + " maps to submesh:" + dgo.targetSubmeshIdxs[i] + " added:" + dgo._submeshTris[i].Length, LOG_LEVEL);
            }
            return true;
        }

        // if adding many copies of the same mesh want to cache obUVsResults
        bool _collectOutOfBoundsUVRects2(Mesh m, DynamicGameObject dgo, Material[] sharedMaterials, OrderedDictionary sourceMats2submeshIdx_map, Dictionary<int, MeshBakerUtility.MeshAnalysisResult[]> meshAnalysisResults, MeshChannelsCache meshChannelCache)
        {
            ...
            MeshBakerUtility.MeshAnalysisResult[] res;
            if (meshAnalysisResults.TryGetValue(m.GetInstanceID(), out res))
            {
                ...
                res = new MeshBakerUtility.MeshAnalysisResult[numTriMeshes];
                for (int i = 0; i < numTriMeshes; i++)
                {
        ...
        bool _showHide(GameObject[] goToShow, GameObject[] goToHide)
        {
            ...
            for (int i = 0; i < goToHide.Length; i++)
            {
                ...
                {
                    if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Trying to hide an object " + goToHide[i] + " that is not in combined mesh. Did you initially bake with 'clear buffers after bake' enabled?");
                    return false;
                }
            }
            ...
            for (int i = 0; i < goToShow.Length; i++)
            {
                ...
                {
                    if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Trying to show an object " + goToShow[i] + " that is not in combined mesh. Did you initially bake with 'clear buffers after bake' enabled?");
                    return false;
                }
            }
        ...
        bool _addToCombined(GameObject[] goToAdd, int[] goToDelete, bool disableRendererInSource)
        {
            ...
            TextureBakeResults.Material2AtlasRectangleMapper mat2rect_map = new TextureBakeResults.Material2AtlasRectangleMapper(textureBakeResults);
            ...
            if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("==== Calling _addToCombined objs adding:" + _goToAdd.Length + " objs deleting:" + _goToDelete.Length + " fixOutOfBounds:" + textureBakeResults.fixOutOfBoundsUVs + " doMultiMaterial:" + textureBakeResults.doMultiMaterial + " disableRenderersInSource:" + disableRendererInSource, LOG_LEVEL);
            ...
            if (textureBakeResults.doMultiMaterial)
            {
                ...
                {
                    MultiMaterial mm = textureBakeResults.resultMaterials[i];
            ...
            List<DynamicGameObject>[] boneIdx2dgoMap = null;
            ...
            for (int i = 0; i < _goToDelete.Length; i++)
            {
                ...
                if (instance2Combined_MapTryGetValue(_goToDelete[i], out dgo))
                {
                    ...
                    if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Trying to delete an object that is not in combined mesh");
                }
            }

            //now add
            List<DynamicGameObject> toAddDGOs = new List<DynamicGameObject>();
                ...
                if (!instance2Combined_MapContainsKey(_goToAdd[i].GetInstanceID()) || Array.FindIndex<int>(_goToDelete, o => o == _goToAdd[i].GetInstanceID()) != -1)
                {
                    ...
                    if (_goToAdd[i] != null)
                    {
                        ...
                        if (lightmapOption == MB2_LightmapOptions.preserve_current_lightmapping)
                        {
                            ...
                            {
                                if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Object " + go.name + " has a different lightmap index. Lightmapping will not work.");
                            }
                            if (!MBVersion.GetActive(go))
                            {
                                if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Object " + go.name + " is inactive. Can only get lightmap index of active objects.");
                            }
                            if (r.lightmapIndex == -1)
                            {
                                if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Object " + go.name + " does not have an index to a lightmap.");
                            }
                        }
                    ...
                    if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Object " + _goToAdd[i].name + " has already been added");
                    _goToAdd[i] = null;
                }
            }
            ...
            for (int i = 0; i < _goToAdd.Length; i++)
            {
                if (_goToAdd[i] != null && disableRendererInSource)
                {
                    ...
                    if (LOG_LEVEL == MBLogLevel.trace) Debug.Log("Disabling renderer on " + _goToAdd[i].name + " id=" + _goToAdd[i].GetInstanceID());
                }
            }
            ...
            if (LOG_LEVEL >= MBLogLevel.debug) Debug.Log("Verts adding:" + totalAddVerts + " deleting:" + totalDeleteVerts + " submeshes:" + newSubmeshTrisSize.Length + " bones:" + newBonesSize + " blendShapes:" + newBlendShapeSize);
            ...
            for (int i = 0; i < newSubmeshTrisSize.Length; i++)
            {
                ...
                if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("    submesh :" + i + " already contains:" + submeshTris[i].Length + " tris to be Added:" + totalAddSubmeshTris[i] + " tris to be Deleted:" + totalDeleteSubmeshTris[i]);
            }
            ...
            for (int i = 0; i < mbDynamicObjectsInCombinedMesh.Count; i++)
            {
                ...
                {
                    if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Copying obj in combined arrays idx:" + i, LOG_LEVEL);
                    ...
                    for (int subIdx = 0; subIdx < numResultMats; subIdx++)
                    {
                        ...
                        if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("    Adjusting submesh triangles submesh:" + subIdx + " startIdx:" + sTriIdx + " num:" + sNumTris, LOG_LEVEL);
                    ...
                    if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Not copying obj: " + i, LOG_LEVEL);
            ...
            {
                DynamicGameObject dgo = toAddDGOs[i];
                GameObject go = _goToAdd[i];
                ...
                {
                    if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Mesh " + dgo.name + " has more submeshes than materials");
                    numTriSets = dgo.uvRects.Length;
                }
                else if (dgo.uvRects.Length > numTriSets)
                {
                    if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Mesh " + dgo.name + " has fewer submeshes than materials");
                }
                ...
                if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Added to combined:" + dgo.name + " verts:" + nverts.Length + " bindPoses:" + nbindPoses.Length, LOG_LEVEL);
            }
            if (lightmapOption == MB2_LightmapOptions.copy_UV2_unchanged_to_separate_rects)
            {
                _copyUV2unchangedToSeparateRects();
            }
            if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("===== _addToCombined completed. Verts in buffer: " + verts.Length, LOG_LEVEL);
            return true;
        }

        void _copyAndAdjustUVsFromMesh(DynamicGameObject dgo, Mesh mesh, int vertsIdx, MeshChannelsCache meshChannelsCache)
        {
            ...
            if (textureBakeResults.fixOutOfBoundsUVs == false)
            {
                ...
                if (allAreIdent)
                {
                    ...
                    if (LOG_LEVEL >= MBLogLevel.debug) Debug.Log("All atlases have only one texture in atlas UVs will be copied without adjusting");
                }
            }
            if (needToModfyUVs)
            {
                ...
                for (int k = 0; k < dgo.targetSubmeshIdxs.Length; k++)
                {
                    ...
                    DRect encapsulatingRectMatAndUVTilingInverse = UVTransformUtility.InverseTransform(ref encapsulatingRectMatAndUVTiling);

                    DRect toNormalizedUVs = UVTransformUtility.InverseTransform(ref obUVRect);

                    DRect meshFullSamplingRect = UVTransformUtility.CombineTransforms(ref obUVRect, ref sourceMaterialTiling);
                    ...
                    DRect relativeTrans = UVTransformUtility.CombineTransforms(ref meshFullSamplingRect, ref encapsulatingRectMatAndUVTilingInverse);

                    // [transform] = [toNormalizedUVs][relativeTrans][uvSubRectInAtlas]
                    DRect trans = UVTransformUtility.CombineTransforms(ref toNormalizedUVs, ref relativeTrans);
                    trans = UVTransformUtility.CombineTransforms(ref trans, ref atlasRect);
            ...
            if (LOG_LEVEL >= MBLogLevel.trace) Debug.Log(string.Format("_copyAndAdjustUVsFromMesh copied {0} verts", nuvs.Length));
        }

        void _copyAndAdjustUV2FromMesh(DynamicGameObject dgo, Mesh mesh, int vertsIdx, MeshChannelsCache meshChannelsCache)
        {
            ...
            { //has a lightmap
                ...
                if (LOG_LEVEL >= MBLogLevel.trace) Debug.Log("_copyAndAdjustUV2FromMesh copied and modify for preserve current lightmapping " + nuv2s.Length);
            }
            else
            {
        ...
        public override void UpdateSkinnedMeshApproximateBoundsFromBones()
        {
            if (outputOption == MB2_OutputOptions.bakeMeshAssetsInPlace)
            {
                if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Can't UpdateSkinnedMeshApproximateBounds when output type is bakeMeshAssetsInPlace");
                return;
            }
            if (bones.Length == 0)
            {
                if (verts.Length > 0) if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("No bones in SkinnedMeshRenderer. Could not UpdateSkinnedMeshApproximateBounds.");
                return;
            }
            if (_targetRenderer == null)
            {
                if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Target Renderer is not set. No point in calling UpdateSkinnedMeshApproximateBounds.");
                return;
            }
            if (!_targetRenderer.GetType().Equals(typeof(SkinnedMeshRenderer)))
            {
                if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Target Renderer is not a SkinnedMeshRenderer. No point in calling UpdateSkinnedMeshApproximateBounds.");
                return;
            }
        ...
        public override void UpdateSkinnedMeshApproximateBoundsFromBounds()
        {
            if (outputOption == MB2_OutputOptions.bakeMeshAssetsInPlace)
            {
                if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Can't UpdateSkinnedMeshApproximateBoundsFromBounds when output type is bakeMeshAssetsInPlace");
                return;
            }
            if (verts.Length == 0 || mbDynamicObjectsInCombinedMesh.Count == 0)
            {
                if (verts.Length > 0) if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Nothing in SkinnedMeshRenderer. Could not UpdateSkinnedMeshApproximateBoundsFromBounds.");
                return;
            }
            if (_targetRenderer == null)
            {
                if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Target Renderer is not set. No point in calling UpdateSkinnedMeshApproximateBoundsFromBounds.");
                return;
            }
            if (!_targetRenderer.GetType().Equals(typeof(SkinnedMeshRenderer)))
            {
                if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("Target Renderer is not a SkinnedMeshRenderer. No point in calling UpdateSkinnedMeshApproximateBoundsFromBounds.");
                return;
            }
        ...
        {
            if (_validationLevel >= MBValidationLevel.quick && !ValidateTargRendererAndMeshAndResultSceneObj()) return;
            if (_mesh != null)
            {
                ...
                if (LOG_LEVEL >= MBLogLevel.trace) Debug.Log("ApplyShowHide");
            }
            else {
                                   ...
                                   GenerateUV2Delegate uv2GenerationMethod = null)
        {
            ...
            if (_mesh != null)
            {
                ...
                if (triangles && _textureBakeResults != null)
                {
                    int[][] submeshTrisToUse = GetSubmeshTrisWithShowHideApplied();
                ...
                if (renderType != MB_RenderType.skinnedMeshRenderer && lightmapOption == MB2_LightmapOptions.generate_new_UV2_layout)
                {
                    if (uv2GenerationMethod != null)
                    {
                        ...
                        if (LOG_LEVEL >= MBLogLevel.trace) Debug.Log("generating new UV2 layout for the combined mesh ");
                    }
                    else {
                ...
                {
                    if (LOG_LEVEL >= MBLogLevel.warn) Debug.LogWarning("UV2 cannot be generated for SkinnedMeshRenderer objects.");
                }
                ...
                if (blendShapesFlag)
                {
                    ...
                    for (int i = 0; i < blendShapes.Length; i++)
                    {
                        DynamicGameObject dgo = instance2Combined_MapGet(blendShapes[i].gameObjectID);
                        if (dgo != null)
                        {
                ...
                {
                    if (LOG_LEVEL >= MBLogLevel.trace) Debug.Log("recalculating bounds on mesh.");
        ...
        {
            if (LOG_LEVEL >= MBLogLevel.debug) Debug.Log("UpdateGameObjects called on " + gos.Length + " objects.");
            _initialize();
            if (_mesh.vertexCount > 0 && _instance2combined_map.Count == 0)
            {
                Debug.LogWarning("There were vertices in the combined mesh but nothing in the MeshBaker buffers. If you are trying to bake in the editor and modify at runtime, make sure 'Clear Buffers After Bake' is unchecked.");
        ...
        public override bool AddDeleteGameObjectsByID(GameObject[] gos, int[] deleteGOinstanceIDs, bool disableRendererInSource)
        {
            ...
            if (validationLevel > MBValidationLevel.none)
            {
                ...
                if (deleteGOinstanceIDs != null && validationLevel >= MBValidationLevel.robust)
                {
        ...
        public override void ClearBuffers()
        {
            ...
            if (LOG_LEVEL >= MBLogLevel.trace) MBLog.LogDebug("ClearBuffers called");
        }
        ...
        public override void DestroyMesh()
        {
            if (_mesh != null)
            {
                if (LOG_LEVEL >= MBLogLevel.debug) MBLog.LogDebug("Destroying Mesh");
        ...
        public bool ValidateTargRendererAndMeshAndResultSceneObj()
        {
            ...
            {
                if (_LOG_LEVEL >= MBLogLevel.error) Debug.LogError("Result Scene Object was not set.");
                return false;
            }
            else {
                if (_targetRenderer == null)
                {
                    if (_LOG_LEVEL >= MBLogLevel.error) Debug.LogError("Target Renderer was not set.");
                    return false;
                }
                else {
                    if (_targetRenderer.transform.parent != _resultSceneObject.transform)
                    {
                        if (_LOG_LEVEL >= MBLogLevel.error) Debug.LogError("Target Renderer game object is not a child of Result Scene Object was not set.");
                        return false;
                    }
                    if (_renderType == MB_RenderType.skinnedMeshRenderer)
                    {
                        ...
                        {
                            if (_LOG_LEVEL >= MBLogLevel.error) Debug.LogError("Render Type is skinned mesh renderer but Target Renderer is not.");
                            return false;
                        }
                        if (((SkinnedMeshRenderer)_targetRenderer).sharedMesh != _mesh)
                        {
                            if (_LOG_LEVEL >= MBLogLevel.error) Debug.LogError("Target renderer mesh is not equal to mesh.");
                            return false;
                        }
                    }
                    if (_renderType == MB_RenderType.meshRenderer)
                    {
                        ...
                        {
                            if (_LOG_LEVEL >= MBLogLevel.error) Debug.LogError("Render Type is mesh renderer but Target Renderer is not.");
                            return false;
                        }
        ...
        public static Renderer BuildSceneHierarch(MeshCombinerSingle mom, GameObject root, Mesh m, bool createNewChild = false, GameObject[] objsToBeAdded = null)
        {
            if (mom._LOG_LEVEL >= MBLogLevel.trace) Debug.Log("Building Scene Hierarchy createNewChild=" + createNewChild);
            GameObject meshGO;
            MeshFilter mf = null;
            ...
            if (mom.renderType == MB_RenderType.skinnedMeshRenderer)
            {
                ...
                if (r != null) MeshBakerUtility.Destroy(r);
                MeshFilter f = meshGO.GetComponent<MeshFilter>();
                if (f != null) MeshBakerUtility.Destroy(f);
        ...
        List<DynamicGameObject>[] _buildBoneIdx2dgoMap()
        {
            List<DynamicGameObject>[] boneIdx2dgoMap = new List<DynamicGameObject>[bones.Length];
            for (int i = 0; i < boneIdx2dgoMap.Length; i++) boneIdx2dgoMap[i] = new List<DynamicGameObject>();
        ...
        void _CollectBonesToAddForDGO(DynamicGameObject dgo, Dictionary<Transform,int> bone2idx, HashSet<int> boneIdxsToDelete, HashSet<BoneAndBindpose> bonesToAdd, Renderer r, MeshChannelsCache meshChannelCache)
        {
        ...
        void _AddBonesToNewBonesArrayAndAdjustBWIndexes(DynamicGameObject dgo, Renderer r, int vertsIdx,
