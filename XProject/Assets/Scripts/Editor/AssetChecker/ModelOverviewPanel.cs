...
namespace AssetChecker
{
    ...
    public class ModelOverviewPanel : IEditorPanel
    {
        ...
        private string bipBone = "Bip";

        private string unNub = "Nub";
        private string unFootsteps = "Footsteps";
        ...
        private ModelBean parseModel(string filePath)
        {
            ...
            modelBean.BondCount = readBoneNumber(filePath);
...
//            if (boneTrans != null) modelBean.BondCount = boneTrans.Length;
        ...
        /// 读取骨骼
        /// </summary>
        private int readBoneNumber(string filePath)
        {
            ModelImporter modelImporter = AssetImporter.GetAtPath(filePath) as ModelImporter;
            string[] bonePaths = modelImporter.transformPaths;

            HashSet<string> boneSet = new HashSet<string>();
            for (int i = 0; i < bonePaths.Length; i++)
            {
                readBone(bonePaths[i] , boneSet);
            }
            //Debug.Log(string.Join("\n" , boneSet.ToArray()));
            return boneSet.Count;
        }

        private void readBone(string path, HashSet<string> boneSet)
        {
            ...
            if (lastIndex < 0)
            {
                if (!boneSet.Contains(path) && isBone(ref path))
                    boneSet.Add(path);
                return;
            }

            string boneName = path.Substring(lastIndex + 1, path.Length - lastIndex - 1);
            if (!boneSet.Contains(boneName) && isBone(ref boneName))
                boneSet.Add(boneName);
            readBone(path.Substring(0, lastIndex), boneSet);
        }


        private bool isBone(ref string boneName)
        {
            if (!boneName.StartsWith(bone) && !boneName.StartsWith(bipBone))
                return false;
            if (boneName.EndsWith(unNub) || boneName.EndsWith(unFootsteps))
                return false;
            return true;
        }
